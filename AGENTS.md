...
- The editor and numeric gutter font should resolve to MonoLisa via `SpotNoteFont.editorFontName == "MonoLisa-Regular"` (a licensed font installed in `~/Library/Fonts`, not redistributed in the repo), falling back to the bundled IBM Plex Mono (`SpotNoteFont.fallbackFontName == "IBMPlexMono"`) and then the system monospaced face; do not silently fall back to Inter or a proportional system font for the HUD text surface.
...
- Markdown outline behavior belongs in `MarkdownOutline.swift` and `PlaceholderTextView`: Enter and Vim normal-mode `o`/`O` continue `- ` bullets with the current indentation, Tab indents a bullet by one two-space level, Shift-Tab outdents by one level, and pressing Enter on an empty bullet exits the list. Motions are split by prefix: the `,` prefix jumps to a section and inserts a fresh `- ` bullet at its end (creating it if absent, ignoring internal blank spacers): `,h` → `## Habits`, `,d` → `## Todo`, `,t` → `## Tray`, `,b` → `## Big Things`. The `g` prefix is reserved for Linear handoff: `gd/gp/gs/gt/gl` send the bullet to David's Personal workspace at Done/Planned/Started/Triage/Later, and `gc` sends it to his Code workspace at Triage with a Build label. The `\` leader handles other sends and is reusable: `\t` appends the current line to tray.md; `\h` logs the current `## Habits` bullet to the Life Dashboard habit tracker (sends the bullet to the loopback ingress, then clears it — habits are re-added daily); `\c` appends the current bullet block(s) to the hermes-build State.md (`~/Documents/knowledge/Work/hermes-build/State.md`) as one flattened `- ` line per block, then clears the source (`N\c` files N separate `- ` lines, not one fused line); and `\f` (or `:fmt`) tidies blank-line spacing around section headers (one blank line above and below each header, none above the top header), preserving bullets and multiline bullets. (Legacy `gH/gD/gT/tt/gy` are retired.)
...
- Confirm editor text and line numbers render in MonoLisa (IBM Plex Mono only if MonoLisa is not installed).
- Confirm `- ` bullets continue with Enter/normal-mode `o`, Tab indents, and Shift-Tab outdents.
- Confirm Title-Case headings; in Vim normal mode, `,h`/`,d`/`,t`/`,b` jump to `## Habits`/`## Todo`/`## Tray`/`## Big Things` and drop onto a fresh `- ` bullet at the end of each (creating the section if absent). `\t` appends the current line to tray.md; `\h` logs the current Habits bullet to the habit tracker and clears it; `\c` appends the current bullet to the hermes-build State.md and clears it; `\f` tidies blank-line spacing around headers.
