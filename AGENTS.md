...
- Confirm the inbox starts with `## TODO`; in Vim normal mode, `gD` creates/jumps to a fresh task bullet above `## TRAY`, and `tt`/`gT` jumps below the last non-empty `## TRAY` item, not to an internal spacer blank.
