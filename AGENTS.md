...
- The HUD is a macOS 26 Liquid Glass shell in the SlashNote capture-panel layout (spec extracted from slashnote.app demo CSS + pixel-sampled renders, 2026-08-08): the card background is `glassEffect` with the theme's near-opaque tint and radius-19 continuous corners. A fixed `GlassToolbar` (`EditorMetrics.toolbarHeight == 40`, 24px circles, white-8% fills, glyphs at 60% text alpha) sits above the editor — left cluster X/doc/pin (dismiss / fuzzy switcher / `dimOnFocusLoss`), right cluster the AA-checklist-list pill (cycle theme / append `- ` bullet / line numbers) plus `#` (find bar). A fixed `ComposerBar` (`EditorMetrics.composerHeight == 48`, "What to write?" capsule) sits below the editor; submit appends a `- ` bullet, mic stays a disabled placeholder. Heights are mirrored in `SpotlightRootView.extraChromeHeight` and the controller's `chromeAboveEditor`/`chromeBelowEditor`. The `slash-dark` theme (#1B1B1B, white-alpha controls) is the palette match; the old `SpotNoteVisualEffectView` NSVisualEffectView wrapper is deleted.
