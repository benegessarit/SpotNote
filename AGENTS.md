...
- Confirm Title-Case headings; in Vim normal mode, `,d`/`,t` jump to `## Todo`/`## Tray` and drop onto a fresh `- ` bullet at the end of each (creating the section if absent). `\t` appends the current line to tray.md; `\c` appends the current bullet to the hermes-build State.md and clears it; `\f` tidies blank-line spacing around headers.
