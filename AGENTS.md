...
- Confirm Title-Case headings; in Vim normal mode, `,h`/`,d`/`,t`/`,b` jump to `## Habits`/`## Todo`/`## Tray`/`## Big Things` and drop onto a fresh `- ` bullet at the end of each (creating the section if absent). `\t` appends the current line to tray.md; `\h` logs the current Habits bullet to the habit tracker and clears it.
