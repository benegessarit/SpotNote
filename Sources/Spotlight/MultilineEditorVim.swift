...
extension PlaceholderTextView {
  ...
  func executeVimAction(_ action: VimAction) {
    ...
    if executeVisualCharacterAction(action) { return }
...
enum VimActionDispatcher {
  ...
  private static func handleModeAction(
    ...
    case .switchToInsert, .switchToNormal:
      // Collapse any lingering visual selection back to that mode's
      // moving caret so Esc/v/V leaves the user at the active edge,
      // not at AppKit's unordered selection start.
      if let caret = view.visualCharacterCaret {
        let clamped = min(caret, (view.string as NSString).length)
        view.setSelectedRange(NSRange(location: clamped, length: 0))
      } else if let caret = view.visualLineCaret {
        let clamped = min(caret, (view.string as NSString).length)
        view.setSelectedRange(NSRange(location: clamped, length: 0))
      }
      view.visualCharacterAnchor = nil
      view.visualCharacterCaret = nil
